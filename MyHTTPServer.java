...
/**
 *
 * MyHTTPServer serves files via path from GET requests or an index file if no path specified. The root file path taken by the server
 * is the directory the program resides in. A starting port may be specified, and when launched, accepts requests and launches
 * individual threads to process requests. A log file of accessses is also kept in the program directory. If the specified
 * file can not be found, an harcoded 404 html response is returned. Likewise if a method other than GET is used, a harcoded
 * not implemented response is returned.
 *
 * @author Tyler Matthews\
 */
public class MyHTTPServer {
   /**
    * The servers logger, configuration setting set during server intialization
    */
   ...
   /**
    * Configure the logger to print out to 'access_log', and to only log the message component of the LogRecord
    * consulted oracle doc: https://docs.oracle.com/cd/E19340-01/820-6767/gcbkk/index.html
    * @throws IOException if there are IO problems opening the file
    */
   private void configureLogger() throws IOException {
      // log file will be access_log, in append mode
      Handler fh = new FileHandler("access_log", true);
      fh.setLevel(Level.INFO); // log at info
      // need a custom formatter, as we don't want all the meta info, just messgae
      ...
      // attach the class logger to the file handler
      logger.addHandler(fh);
      // don't use parent formatters or handlers
      logger.setUseParentHandlers(false);
   }

   /**
    * Starts up the server, initializing the thread pool, whose threads will handle incoming requests
    * Most of this is from the course text, modified to only use port instead of messing/intializing
    * a root directory, keeping it simple.
    */
   public void start() {
      ...
      try (ServerSocket server = new ServerSocket(this.port)) { // setup server socket
         System.out.println("Accepting connections on port " + server.getLocalPort()); // don't need to log, just to console
         while (true) {
            try {
   ...
   /**
    * Callable invoked in the thread pool to handle servers incoming http request. Modified from
    * the original to track the original accesstime as well as the response code and response size.
    * Once the file is served or an error page displayed, an access log is written with the information.
    *
    * @author Tyler Matthews
    */
   private class HTTPHandler implements Callable<Void> {
      ...
      /**
       * Handles the http request by parsing the request and either serving a file or returning an appropriate error
       */
      @Override
      public Void call() throws IOException {
         ...
         try (BufferedOutputStream rawOutput = new BufferedOutputStream(connection.getOutputStream());
            ...
            HTTPRequestContainer httpRequest = new HTTPRequestContainer(getRequest(in)); // parses out the request line for easy lookup
            if (httpRequest.isGET()) { // if a valid get, try to serve file
               ...
               writeError(rawOutput, HttpURLConnection.HTTP_NOT_IMPLEMENTED, HTML501NOT_IMPLEMENTED_RETURN, httpRequest);
            }
            // after the an error or file has been served, record the access/attempt in the log, using hte same format from last assignment
            logger.info(((InetSocketAddress) connection.getRemoteSocketAddress()).getHostName() + " - - ["
                  + new SimpleDateFormat("dd/MMM/yyyy:hh:mm:ss Z").format(new Date(accessTime)) + "] \"" + httpRequest.requestLine
                  + "\" " + code + " " + responseSize + "\n");
      ...
      /**
       * Attempts to serve the requested file. If no file specified, the index is served.
       *
       ...
       */
      private void serveFile(BufferedOutputStream rawOutput, String fileName, HTTPRequestContainer httpRequest) throws IOException {
         ...
         if (file.canRead()) { // check if file is accessbile
            // jhttp
            byte[] fileBytes = Files.readAllBytes(file.toPath()); // need to premptively read, so we can get the byte size for response header
            writeHeaderIfApplicable(rawOutput, HttpURLConnection.HTTP_OK, getContentType(fileName), fileBytes.length, httpRequest); // write header
            rawOutput.write(fileBytes); // write file
         } else {
            writeError(rawOutput, HttpURLConnection.HTTP_NOT_FOUND, HTML404NOT_FOUND_RETURN, httpRequest); // if file inaccessbile, write 404 page
         }
      }
       ...
       */
      private void writeError(BufferedOutputStream out, int responseIntCode, String error, HTTPRequestContainer httpRequest)
            throws IOException {
      ...
      /**
       * A container to hold the request information, after being pased from the request.
       * @author Tyler Matthews
       */
      ...
      private String getRequest(InputStream in) throws IOException {
         ...
         System.out.println(connection.getRemoteSocketAddress() + " " + request.toString()); // don't log, just to console
         return request.toString();
      }

      /**
       * Writes out a response header if the request was at least http 1, assigns the code and response size for later logging
      ...
      /**
       * Returns the response code string based on the integer ... played with includiing this in the actual header as a method param,
       * but decided this was a little cleaner.
       * @param code The response code
